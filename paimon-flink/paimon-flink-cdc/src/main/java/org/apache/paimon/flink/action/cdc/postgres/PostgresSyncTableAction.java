 ...
 * </ul>
 */
public class PostgresSyncTableAction extends ActionBase {
    private final String database;
    private final String table;
    private final Configuration postgresConfig;
    ...
    public PostgresSyncTableAction(
        ...
        this.table = table;
        this.postgresConfig = Configuration.fromMap(postgresConfig);
    }

    public PostgresSyncTableAction withPartitionKeys(String... partitionKeys) {
        return withPartitionKeys(Arrays.asList(partitionKeys));
    }

    public PostgresSyncTableAction withPartitionKeys(List<String> partitionKeys) {
        this.partitionKeys = partitionKeys;
        return this;
    }

    public PostgresSyncTableAction withPrimaryKeys(String... primaryKeys) {
        return withPrimaryKeys(Arrays.asList(primaryKeys));
    }

    public PostgresSyncTableAction withPrimaryKeys(List<String> primaryKeys) {
        this.primaryKeys = primaryKeys;
        return this;
    }

    public PostgresSyncTableAction withTableConfig(Map<String, String> tableConfig) {
        this.tableConfig = tableConfig;
        return this;
    }

    public PostgresSyncTableAction withComputedColumnArgs(List<String> computedColumnArgs) {
        this.computedColumnArgs = computedColumnArgs;
        return this;
    }

    public PostgresSyncTableAction withTypeMapping(TypeMapping typeMapping) {
        this.typeMapping = typeMapping;
        return this;
    }

    public PostgresSyncTableAction withMetadataKeys(List<String> metadataKeys) {
    ...
    public void build() throws Exception {
        checkArgument(
                postgresConfig.contains(PostgresSourceOptions.TABLE_NAME),
                String.format(
        ...
        CdcMetadataConverter[] metadataConverters =
                metadataColumn.stream()
                        .map(
                                key ->
                                        Stream.of(PostgresMetadataProcessor.values())
                                                .filter(m -> m.getKey().equals(key))
                                                .findFirst()
                                                .orElseThrow(IllegalStateException::new))
                        .map(PostgresMetadataProcessor::getConverter)
    ...
    private Predicate<String> monitorTablePredication() {
        return tableName -> {
            Pattern tableNamePattern =
                    Pattern.compile(postgresConfig.get(PostgresSourceOptions.TABLE_NAME));
    ...
    public void run() throws Exception {
        build();
        execute(env, String.format("PostgreSQL-Paimon Table Sync: %s.%s", database, table));
    }
}
