...
/**
 * Enumerates the metadata processing behaviors for PostgreSQL CDC related data.
 *
 * <p>This enumeration provides definitions for various PostgreSQL CDC metadata keys along with
 * their associated data types and converters. Each enum entry represents a specific type of
 * metadata related to PostgreSQL CDC and provides a mechanism to read and process this metadata
 * from a given {@link JsonNode} source.
 *
 ...
 */
public enum PostgresMetadataProcessor {
    ...
    SCHEMA_NAME(
            ...
            new CdcMetadataConverter() {
                ...
                public Map<String, String> read(JsonNode record) {
                    String table =
                            record.get("source").get(AbstractSourceInfo.SCHEMA_NAME_KEY).asText();
                    return Collections.singletonMap("SCHEMA_NAME", table);
                }
    ...
    PostgresMetadataProcessor(String key, DataType dataType, CdcMetadataConverter converter) {
        this.key = key;
